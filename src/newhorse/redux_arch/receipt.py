...
class ResidualEvent:
    ...
    # ★ THE CLICK POOL: target ADMISSIONS by origin, counted in `ClickProber.__init__` and `.refresh`. A separate
    # denominator from the branch split (admissions, not steps) and therefore a separate dict -- it is the direct
    # receipt for how much of the untried branch the CONSTRUCTION pool can force before `refresh` contributes at
    # all. See docs/tether/EVIDENCE_the_click_pool_floor.md.
    decide_click_pool: Dict[str, int] = field(default_factory=dict)
...
def summary(events: List[ResidualEvent]) -> Dict[str, Any]:
    ...
    return dict(minted_keys={k: sorted(v) for k, v in sorted(minted_keys.items())},
                ...
                decide_funnel=dict(calls=dec_calls,
                                   ...
                                   # ★ THE CLICK POOL. Deliberately NOT part of the identity above: these are
                                   # ADMISSIONS (targets entering the pool), the branch counts are STEPS, and a
                                   # count offered against the wrong denominator is the defect this whole block
                                   # exists to refuse. `ctor_residue` is the pool's own identity -- the two
                                   # origins must sum to the targets actually admitted after de-dup, so a lattice
                                   # point that collided with a perceptual centroid is visible instead of double
                                   # counted.
                                   click_pool=dict(sorted(dec_click_pool.items())),
                                   click_pool_ctor_residue=(dec_click_pool.get("ctor_targets", 0)
                                                            - dec_click_pool.get("ctor_perceptual", 0)
                                                            - dec_click_pool.get("ctor_sweep", 0)),
