...
def _E():
    try:
        from engines.egocentric import effects
...
class TestTheConstructor:
    ...
    def test_canonical_keying_is_translation_invariant(self):
        ...
        abstraction choice."""
        E = _E()
        b1 = np.zeros((7, 7), dtype=int); b1[1, 1] = 3
        a1 = b1.copy(); a1[1, 1] = 4
        b2 = np.zeros((7, 7), dtype=int); b2[5, 4] = 3
        a2 = b2.copy(); a2[5, 4] = 4
    ...
    def test_different_mechanisms_get_different_keys(self):
        E = _E()
        b = np.zeros((5, 5), dtype=int); b[2, 2] = 3
        a_recolour = b.copy(); a_recolour[2, 2] = 4
        a_vanish = b.copy(); a_vanish[2, 2] = 0
...
class TestTypedGamma:
    ...
    def test_a_composite_is_keyed_and_reusable_as_an_atom(self, tmp_path):
        ...
        g = self._g(tmp_path)
        b = np.zeros((5, 5), dtype=int); b[2, 2] = 3
        mid = b.copy(); mid[2, 2] = 4
        end = mid.copy(); end[2, 2] = 5
...
class TestTheN1Gate:
    ...
    def test_the_effect_beats_the_route_on_one_example(self):
        ...
        E = _E()
        before = np.zeros((6, 6), dtype=int); before[3, 3] = 3
        after = before.copy(); after[3, 3] = 4
        route = [(1,), (1,), (6, 3, 3)]
