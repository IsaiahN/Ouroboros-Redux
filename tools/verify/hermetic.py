"""hermetic.py -- BUILDER-ONLY. Run v4 in isolation, so that two runs can be COMPARED at all.

WHY THIS EXISTS. Beat 2 established two facts that together make every unqualified v4 number unreadable:

  1. v4 IS NOT DETERMINISTIC. Two identical invocations gave 74 and 69 actions, diverging at step 1.
     Nothing in the run path ever seeds an RNG.
  2. RUNS ARE NOT INDEPENDENT. core_data.db is 43 MB and survives every run, carrying
     winning_sequences, curriculum_progress and agent_game_diversity. Replicate N continues
     replicate N-1 rather than sampling it again -- and a replicate that hits [REPLAY-MODE] replays a
     sequence an earlier replicate discovered, which is legitimate learning but is NOT a second sample.

Redux's comparison method assumed determinism: `seq_sha` identity was its correctness check. Here that
check fails on an UNCHANGED system, so cut_wire-by-byte-identity cannot be run on v4 until this is
fixed. This module fixes it without modifying one line of agent code.

HOW, and why each piece is the least invasive option available:

  * FRESH DATABASE, by running in a SANDBOX CWD. Every db_path in the codebase defaults to the
    RELATIVE string "core_data.db", so a subprocess whose cwd is an empty directory creates its own.
    The repo goes on PYTHONPATH so imports still resolve. No env var covers this -- only
    database_logger.py honours DATABASE_PATH; the rest hardcode the default -- and moving the
    production DB aside would lose it if a run crashed.
  * SEEDED RNG, by an injected `sitecustomize.py`. Python imports sitecustomize at interpreter
    startup, BEFORE any agent module loads, so random/numpy/torch are seeded earlier than any agent
    code could seed them. Nothing is patched and nothing is imported by the agent that would not
    otherwise be.
  * PYTHONHASHSEED pinned, because Redux was bitten by a veto keyed on a salted hash.

!! WHAT THIS DOES NOT MAKE HERMETIC, STATED SO IT IS NOT ASSUMED AWAY.
The GAME SERVER is remote and shared. Identical actions should give identical frames, but that is an
assumption about someone else's service, not a property of this harness. If two seeded runs still
differ, the residual source is either the network or a non-RNG nondeterminism (dict/set iteration over
unstable keys, threads, wall-clock reads) -- and `check` below is what distinguishes "seeding worked"
from "seeding was not the whole story". Run `check` before trusting any comparison.

    python tools/verify/hermetic.py check  --game ar25 [--actions 60]     two seeds, same and different
    python tools/verify/hermetic.py run    --game ar25 --seed 1 [--actions 500]
    python tools/verify/hermetic.py band   --game ar25 --reps 5           independent draws, fresh DB each
"""
...
ACTION = re.compile(r"^\s+\[\s*\d+\]\s+(ACTION\d)(?:\s+@\((\d+),(\d+)\))?", re.M)
RESULT = re.compile(r"^\s+([a-z0-9]{4})-[0-9a-f]+:\s*\[(\d+)/(\d+)\]\s*score=([\d.]+)\s*actions=(\d+)",
                    re.M)
REPLAY = re.compile(r"\[REPLAY-MODE\]")
NOGOAL = re.compile(r"no-goal")
REASON = re.compile(r"^\[\s*\d+\]\s+P:", re.M)
...
def _read_probe(box: str):
    p = os.path.join(box, "goal_probe.json")
...
def _sandbox(tag: str, reuse: bool = False) -> str:
    d = os.path.join(RUNS, "hermetic", tag)
    if os.path.isdir(d) and not reuse:
        shutil.rmtree(d, ignore_errors=True)
    os.makedirs(d, exist_ok=True)
    # COMPILE IT BEFORE WRITING IT. A syntax error in this injected file makes Python skip
    # sitecustomize and run the agent WITH NO PROBE AND NO SEED -- which happened once, and produced a
    # complete, plausible-looking run whose only symptom was a missing output file. An instrument that
    # can fail silently is worse than no instrument.
    try:
...
def run(game: str, seed: int, actions: int, mode: str = "normal", tag: str = None,
        timeout: int = 1800, cutwire: str = "", goalprobe: bool = False,
        reuse: bool = False, wipe: bool = False, fc: str = "", cwa: str = "",
        mkt=None, cw_mkt: str = "", eo=None, cw_timer: str = "", cw_prog: str = "", cw_resid: str = "", cw_pariah: str = "",
        tally_decomp: bool = False) -> dict:
    """One fully isolated run. Fresh database, pinned seed, own directory."""
    tag = tag or ("%s_s%d%s" % (game, seed, ("_" + cutwire) if cutwire else ""))
    box = _sandbox(tag, reuse=reuse)
    env = dict(os.environ)
    env["PYTHONPATH"] = os.pathsep.join([REPO, box])
    env["OURO_SEED"] = str(seed)
    if cutwire:
        env["OURO_CUTWIRE"] = cutwire
    if goalprobe:
        env["OURO_GOALPROBE"] = "1"
    if wipe:
        env["OURO_WIPE_LOADED"] = wipe if isinstance(wipe, str) else "1"
    if fc:
        env["OURO_CW_FC"] = fc
    if cwa:
        env["OURO_CW_ACTIONS"] = cwa
    # Tri-state since the default flip: None = leave unset (the shipped default, now ON);
    # True = explicit "1"; False = explicit "0" (opt-out to the legacy path).
    if mkt is not None:
        env["OURO_MARKETPLACE"] = "1" if mkt else "0"
    if eo is not None:
        env["OURO_EARNED_OVERRIDE"] = "1" if eo else "0"
    if cw_mkt:
        env["OURO_CW_MKT"] = cw_mkt
    if cw_timer:
        env["OURO_CW_TIMER"] = cw_timer
    if cw_prog:
        env["OURO_CW_PROGRESS"] = cw_prog
    if cw_resid:
        env["OURO_CW_RESID"] = cw_resid
    if cw_pariah:
        env["OURO_CW_PARIAH"] = cw_pariah
    if tally_decomp:
        env["OURO_TALLY_DECOMP"] = "1"
    env["PYTHONHASHSEED"] = str(seed)
    log = os.path.join(box, "run.log")
    cmd = [PY, os.path.join(REPO, "evolution_runner.py"), "--mode", mode, "--game", game,
           "--population", "1", "--max-generations", "1", "--games-per-gen", "1",
           "--max-actions", str(actions), "--verbose"]
    t0 = time.time()
    status = "ok"
    try:
    ...
    acts = ["%s%s" % (m[0], ("@%s,%s" % (m[1], m[2])) if m[1] else "") for m in ACTION.findall(text)]
    rows = [r for r in RESULT.findall(text) if r[0] == game]
    reasoning = len(REASON.findall(text))
    fresh_db = os.path.exists(os.path.join(box, "core_data.db"))
    # Reused boxes overwrite run.log per episode; keep a per-seed copy so lifecycle
    # episodes remain individually readable (beat 77's instrument limit).
    try:
        import shutil
...
def check(game: str, actions: int) -> int:
    """Does seeding buy determinism? Two runs at one seed, one run at another."""
    print("HERMETIC CHECK -- game=%s, %d actions. Fresh DB and pinned seed on every run." % (game, actions))
    a = run(game, 1, actions, tag="chk_s1a")
    b = run(game, 1, actions, tag="chk_s1b")
    c = run(game, 2, actions, tag="chk_s2")
    for lbl, r in (("seed 1, run A", a), ("seed 1, run B", b), ("seed 2", c)):
        print("  %-14s %-18s %3d action lines  levels=%s  %s%s"
              % (lbl, r["seq_sha"], r["action_lines"], r["levels"], r["status"],
                 "  [REPLAYED]" if r["replayed"] else ""))
        if not r["fresh_db"]:
            print("      !! no core_data.db in the sandbox -- ISOLATION DID NOT HAPPEN, results void")

    same = a["seq_sha"] == b["seq_sha"]
    print()
    if same:
        print("  SAME SEED -> IDENTICAL. Seeding buys determinism; byte-comparison is valid here.")
    else:
        print("  SAME SEED -> STILL DIFFERS. Seeding is NOT sufficient.")
        print("  Do NOT compare runs byte-for-byte. The residual source is the remote game server or a")
        print("  non-RNG nondeterminism (unstable dict/set iteration, threads, wall-clock). Compare")
        print("  DISTRIBUTIONS over replicates instead, and size the band before reading any delta.")
    if a["seq_sha"] == c["seq_sha"] and same:
        print("  !! DIFFERENT SEEDS ALSO IDENTICAL -- the seed is not reaching the decision path.")
        print("     A 'deterministic' result here would be determinism by inertness, not by control.")
    return 0 if same else 1


def band(game: str, reps: int, actions: int) -> int:
    """Independent draws: fresh DB and a different seed each time. This is what a band means here."""
    print("BAND -- game=%s, %d independent replicates, fresh DB and distinct seed each." % (game, reps))
    rs = []
    for i in range(reps):
        r = run(game, 100 + i, actions, tag="band_%s_%d" % (game, i))
        rs.append(r)
        ng = ("%d/%d" % (r["no_goal"], r["reasoning_lines"])) if r["reasoning_lines"] else "-"
        print("  rep %d  seed=%3d  levels=%-4s actions=%-5s no-goal=%-10s %s%s"
              % (i, r["seed"], r["levels"], r["action_lines"], ng, r["status"],
                 "  [REPLAYED -- not an independent draw]" if r["replayed"] else ""))
    lv = [r["levels"] for r in rs if r["levels"] is not None]
    out = os.path.join(RUNS, "band_%s.json" % game)
    with open(out, "w", encoding="utf-8") as fh:
        json.dump(rs, fh, indent=2)
    print()
    if lv:
        print("  levels over %d draws: min=%d max=%d  -> ANY DELTA OF %d OR LESS IS NOISE ON THIS GAME."
              % (len(lv), min(lv), max(lv), max(lv) - min(lv)))
    print("  written to %s" % out)
    return 0


def cutwire(games, seed: int, actions: int) -> int:
    """The pre-registered experiment: does has_goal steer anything? docs/carryover/PREREG_has_goal_cutwire.md"""
    print("CUT-WIRE on has_goal -- seed=%d, %d actions, hermetic (fresh DB + pinned seed per run)."
          % (seed, actions))
    print("%-6s %-18s %-18s %-18s %s" % ("game", "control", "force-high", "negate", "verdict"))
    verdicts = []
    for g in games:
        r = {m: run(g, seed, actions, tag="cw_%s_%s" % (g, m or "ctl"), cutwire=m)
             for m in ("", "force", "negate")}
        c = r[""]
        same = [m for m in ("force", "negate") if r[m]["seq_sha"] == c["seq_sha"]]
        # GATE 1 from the prereg: the corruption must actually reach the flag, or the arm is a
        # BUILD DEFECT rather than a result. The forced arm's own log must show no-goal collapse.
        reached = r["force"]["no_goal"] == 0 and r["force"]["reasoning_lines"] > 0
        # A VACUOUS FORCE ARM. If the control ALREADY has has_goal true on every step, forcing it
        # true is a no-op BY CONSTRUCTION and its byte-identity says nothing whatsoever. Before the
        # frame fix the control was mostly no-goal and the arm was meaningful; after it, on the games
        # the fix rescued, it is a tautology -- and this tool reported the tautology as "WIRE CUT".
        # Label it rather than let a reader count it as evidence.
        force_vacuous = c["reasoning_lines"] > 0 and c["no_goal"] == 0
        neg_same = "negate" in same
        if not reached:
            v = "!! HOOK DID NOT REACH THE FLAG -- build defect, not a result"
        elif force_vacuous:
            v = ("NEGATE BITES -- signal IS consumed" if not neg_same
                 else "negate identical -- signal produced but NOT consumed")
            v += "   [force arm VACUOUS: control is already all-goal, forcing is a no-op]"
        elif len(same) == 2:
            v = "WIRE CUT -- both corruptions byte-identical"
        elif same:
            v = "WIRE CUT (%s identical)" % "+".join(same)
        else:
            v = "listening"
        # GATE 6 from the prereg: an episode too short to test anything is reported, not pooled.
        if c["action_lines"] < 40:
            v += "   [control only %d actions -- too short to pool]" % c["action_lines"]
        print("%-6s %-18s %-18s %-18s %s" % (g, c["seq_sha"], r["force"]["seq_sha"],
                                             r["negate"]["seq_sha"], v))
        print("       control no-goal %d/%d | forced no-goal %d/%d | actions %d/%d/%d"
              % (c["no_goal"], c["reasoning_lines"], r["force"]["no_goal"],
                 r["force"]["reasoning_lines"], c["action_lines"], r["force"]["action_lines"],
                 r["negate"]["action_lines"]))
        verdicts.append((g, v))
    out = os.path.join(RUNS, "cutwire_has_goal.json")
    with open(out, "w", encoding="utf-8") as fh:
        json.dump([{"game": g, "verdict": v} for g, v in verdicts], fh, indent=2)
    print()
    print("seq_sha identity is a CORRECTNESS CHECK, not a success. 'WIRE CUT' means NOT CONSUMED.")
    print("written to %s" % out)
    return 0


def goalprobe(games, seed: int, actions: int) -> int:
    """WHY does the goal detector never fire? Distinguish not-reached / early-return / threw / no-match."""
    print("GOAL-CHANNEL PROBE -- seed=%d, %d actions, hermetic. Counts only; changes nothing." % (seed, actions))
    print("%-6s %7s %7s %9s %9s %7s %7s %7s %7s %7s" %
          ("game", "calls", "early", "noScene", "noFrame", "threw", "refpanl", "transf", "nomatch",
           "setTrue"))
    rows = []
    for g in games:
        r = run(g, seed, actions, tag="gp_%s" % g, goalprobe=True)
        t = r["goal_probe"]
        if t is None:
            print("%-6s  NO PROBE FILE -- the run died before atexit, or the patch missed. NOT a zero." % g)
            continue
        print("%-6s %7d %7d %9d %9d %7d %7d %7d %7d %7d%s"
              % (g, t["calls"], t["early_return"], t.get("early_no_scene", -1),
                 t.get("early_no_frame", -1), t["threw"], t["ref_panel"],
                 t["transforms_only"], t["no_match"], t["set_true"],
                 ("   exc=%s" % t["exc_types"]) if t["exc_types"] else ""))
        print("        SPATIAL: calls=%d ok=%d  guard(cortex=%d frame_list=%d)  SWALLOWED-EXC=%d"
              % (t.get("sp_calls", -1), t.get("sp_ok", -1), t.get("sp_guard_cortex", -1),
                 t.get("sp_guard_framelist", -1), t.get("sp_threw", -1)))
        for k, n in sorted(t.get("vc_exc", {}).items(), key=lambda kv: -kv[1])[:3]:
            print("        VisualCortex.analyze raised %5d x  %s" % (n, k))
        for k, n in sorted(t.get("vc_shapes", {}).items(), key=lambda kv: -kv[1])[:4]:
            print("        input shape %-18s x %d" % (k, n))
        print("        CHAIN: goal_cells nonempty=%d empty=%d | set_goal=%d | plan_to_goal called=%d "
              "returned-a-plan=%d"
              % (t.get("cells_nonempty", -1), t.get("cells_empty", -1), t.get("set_goal", -1),
                 t.get("plan_to_goal_calls", -1), t.get("plan_to_goal_nonempty", -1)))
        print("        DEADLOCK: has_plan_true=%s | map_completeness max=%.2f over-0.5=%s | "
              "fraction_done max=%.2f over-0.3=%s | BOTH(escape open)=%s"
              % (t.get("has_plan_true", 0), t.get("mc_max", -1), t.get("mc_over_half", 0),
                 t.get("fd_max", -1), t.get("fd_over_third", 0), t.get("escape_open", 0)))
        print("        SHADOW PLAN (probe-only, restored): attempts=%s nonempty=%s empty=%s "
              "maxlen=%s no_current_cells=%s threw=%s%s"
              % (t.get("shadow_attempts", 0), t.get("shadow_plan_nonempty", 0),
                 t.get("shadow_plan_empty", 0), t.get("shadow_plan_maxlen", 0),
                 t.get("shadow_no_current_cells", 0), t.get("shadow_threw", 0),
                 ("  exc=%s" % t.get("shadow_exc")) if t.get("shadow_exc") else ""))
        print("        WHY EMPTY: delta==0 on %s steps, delta>0 on %s (max delta %s) | "
              "goal_cells<=%s current_cells<=%s | learned effects<=%s colour cycles<=%s"
              % (t.get("delta_zero", 0), t.get("delta_nonzero", 0), t.get("delta_max", 0),
                 t.get("goal_cells_n", 0), t.get("curr_cells_n", 0),
                 t.get("effects_n", 0), t.get("cycles_n", 0)))
        print("        KEY SPACES: goal%s current%s cycles%s effects%s"
              % (t.get("rng_goal"), t.get("rng_current"), t.get("rng_cycles"), t.get("rng_effects")))
        print("        INTERSECTION (max over run): goal&cycles=%s goal&effects=%s | "
              "DELTA&cycles=%s DELTA&effects=%s"
              % (t.get("isect_goal_cycles", 0), t.get("isect_goal_effects", 0),
                 t.get("isect_delta_cycles", 0), t.get("isect_delta_effects", 0)))
        for k, n in sorted(t.get("teardown_exc", {}).items(), key=lambda kv: -kv[1])[:4]:
            print("        TEARDOWN %5d x  %s" % (n, k))
        st = t.get("strategies", {})
        if st:
            print("        STRATEGY: %s" % ", ".join("%s=%d" % kv for kv in
                                                     sorted(st.items(), key=lambda kv: -kv[1])[:5]))
        rows.append({"game": g, **t, "actions": r["action_lines"], "levels": r["levels"]})
    out = os.path.join(RUNS, "goal_probe.json")
    with open(out, "w", encoding="utf-8") as fh:
        json.dump(rows, fh, indent=2)
    print()
    print("calls=0 means perceive() never reached the channel. early>0 means it returned before any")
    print("detection. threw>0 means the swallowed except-Exception path fired. no_match means it ran")
    print("and genuinely found neither a reference panel nor a transformation -- a DETECTION RULE")
    print("problem, not a plumbing problem. These are different bugs and only this table separates them.")
    print("written to %s" % out)
    return 0
...
def main() -> int:
    ...
    ap.add_argument("cmd", choices=["check", "run", "band", "cutwire", "goalprobe"])
    ...
    if not os.environ.get("ARC_API_KEY"):
        print("ARC_API_KEY not in environment -- refusing to run. Never read from a file here.")
        return 2

    if a.cmd == "check":
        return check(a.game, a.actions)
    if a.cmd == "band":
        return band(a.game, a.reps, a.actions)
    if a.cmd == "goalprobe":
        return goalprobe([g.strip() for g in a.game.split(",") if g.strip()], a.seed, a.actions)
    if a.cmd == "cutwire":
        return cutwire([g.strip() for g in a.game.split(",") if g.strip()], a.seed, a.actions)
    r = run(a.game, a.seed, a.actions, mode=a.mode)
