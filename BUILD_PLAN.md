...
- **Suite: 58/58.** Next: brick 8 hard-bound enforcers at import (reset-earned-only, no-downsampling as import-time guards, not dials); then brick 9 run vs one clean ARC-3 game through the live gate (metric: levels_completed only).
