...
- **Suite: 69/69.** Next: brick 9 — run the horse vs one clean ARC-3 game through the live gate. Metric: `levels_completed` only (no fractional goose chase). ARC key env-only, never written to any file. RESET stays earned-only in active play (now structurally enforced by brick 8).
