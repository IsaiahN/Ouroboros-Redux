...
- **Suite: 49/49.** Next: brick 7 loop closure (§9.6 — wire PERCEIVE→THINK→MAP→ACT with both feedback arrows, integrating bricks 1–6 into one small COMPLETE loop; monolith-then-carve). Then 8 hard-bound enforcers at import; 9 run vs one clean ARC-3 game (metric: levels_completed).
